...
type NumericField = { key: string; label: string; value: string; info: string; type: 'core' | 'rated'; defaultValue?: number; higherBetter?: boolean; targetValue?: string; targetLabel?: string; };
type YesNoField = { key: string; label: string; value: YesNoValue; info: string; };
type NumericFullscreenProps = { numericState: NumericField[]; updateNumericValue: (key: string, newValue: string, isTarget?: boolean) => void; computeGap: (f: NumericField) => number; computeRating: (f: NumericField) => number; handleNumericSubmit: () => void; setView: React.Dispatch<React.SetStateAction<View>>; isDesktop: boolean; };
type YesNoFullscreenProps = { yesNoState: YesNoField[]; updateYesNoValue: (key: string, checked: YesNoValue) => void; handleYesNoSubmit: () => void; setView: React.Dispatch<React.SetStateAction<View>>; isDesktop: boolean; };
type ResultsFullscreenProps = { finalScore: number; numericScoreWeighted: number; yesScoreWeighted: number; setView: React.Dispatch<React.SetStateAction<View>>; onOpenReport: () => void; };
type LandingViewProps = { numericDone: boolean; yesNoDone: boolean; setView: React.Dispatch<React.SetStateAction<View>>; };
type CollapsibleSectionProps = { title: string; icon: React.ElementType; isComplete: boolean; statusText: string; isOpen: boolean; setIsOpen: React.Dispatch<React.SetStateAction<boolean>>; children: React.ReactNode; isDisabled?: boolean; };
type ReportModalProps = { isOpen: boolean; onClose: () => void; tips: string[]; user: AppUser | null; onUnlockRequest: () => void; };
...
const InfoPopup = ({ info, isDesktop }: { info: string; isDesktop: boolean }) => {
    ...
    const commonContentClasses = "w-80 z-[101] bg-black text-white border-black p-2 rounded-md";
                    ...
                    {fields.map(f => (
                      <TableRow key={f.key} className="block md:table-row mb-4 md:mb-0 border rounded-lg md:border-none p-2 md:p-0">
                        ...
                        <TableCell className="block md:table-cell" data-label="Your Value">
                          <Input type="text" value={f.key !== "familyMembers" ? formatCurrency(f.value) : f.value} onChange={e => updateNumericValue(f.key, e.target.value)} className="w-full" placeholder={"Enter value"} />
                        </TableCell>
                        {f.type === "rated" ? (
                          <>
                            <TableCell className="block md:table-cell text-left md:text-center" data-label="Target">
                                {f.targetLabel ? (
                                    <Input type="text" value={formatCurrency(f.targetValue || "")} onChange={e => updateNumericValue(f.key, e.target.value, true)} className="w-full" placeholder={f.targetLabel}/>
                                ) : (
                                    <Input type="text" value={formatCurrency(f.defaultValue || 0)} disabled className="w-full bg-gray-100 border-gray-300 text-gray-700 text-center disabled:cursor-default"/>
                                )}
                            </TableCell>
                            <TableCell className="block md:table-cell text-left md:text-center font-medium" data-label="Gap">
                              {(() => {
                                ...
                                return (<span className={colorClass}>{formatCurrency(Math.abs(gap))}</span>);
                              })()}
                            </TableCell>
                ...
                {fields.map(f => {
                  ...
                  return (
                    <div key={f.key} className="border rounded-xl p-4 bg-white shadow-sm">
                      ...
                      <Input type="text" value={f.key !== "familyMembers" ? formatCurrency(f.value) : f.value} onChange={e => updateNumericValue(f.key, e.target.value)} placeholder={"Your current value"} className="mb-3"/>
                      {f.targetLabel && ( <Input type="text" value={formatCurrency(f.targetValue || "")} onChange={e => updateNumericValue(f.key, e.target.value, true)} placeholder={f.targetLabel} className="mb-3"/> )}
                      {f.type === "rated" && (
                        <div className="grid grid-cols-3 gap-3 text-sm text-center">
                          ...
                          <div>
                            ...
                            <p className={`font-semibold ${colorClass}`}>{formatCurrency(Math.abs(gap))}</p>
                          </div>
                          <div>
        ...
        <CardContent>
          <div className="flex flex-col items-center gap-4">
            <div className={`text-center p-3 rounded-lg font-semibold text-lg w-full max-w-md ${finalScore >= 50 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                {finalScore >= 50 ? `Congratulations! You've passed the financial health check.` : `It looks like there's room for improvement.`}
            </div>
            <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md mt-3">
              <Button variant="ghost" onClick={() => setView("landing")} className="w-full sm:w-1/2 border border-gray-200 hover:bg-gray-100">Back to Home</Button>
              <Button onClick={onOpenReport} className="w-full sm:w-1/2 bg-blue-600 hover:bg-blue-700 text-white">Download Report</Button>
            </div>
          </div>
          <div className="mt-6 text-xs text-gray-500">
            <p><strong>Notes:</strong></p>
            <ul className="ml-5 list-disc"><li>Your score is an estimate based on standard financial benchmarks.</li></ul>
          </div>
        </CardContent>
      </Card>
    </div>
...
const LandingView = ({ setView, numericDone, yesNoDone }: LandingViewProps) => {
    ...
    return (
        <div className="flex flex-col items-center min-h-screen px-4 pt-12 pb-24 bg-[#fdfbf7] text-black">
            ...
            <CollapsibleSection title="Step 2: Pre-Requisites" icon={CheckSquare} isComplete={yesNoDone} statusText={numericDone ? (yesNoDone ? '✓ Completed' : 'Required') : 'Locked'} isOpen={isYesNoOpen} setIsOpen={setIsYesNoOpen} isDisabled={!numericDone}>
                <p className="mb-4 text-gray-600">Answer a few simple questions about your financial habits and planning.</p>
                <Button onClick={handleYesNoClick} className="w-full sm:w-auto" disabled={!numericDone}>{yesNoDone ? 'Edit Pre-Requisites' : 'Start Step 2'}</Button>
            </CollapsibleSection>
        ...
        <div id="how-it-works" className="w-full max-w-5xl px-4 py-12 mx-auto text-center">
             ...
             <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                {steps.map((step, index) => (
                    <motion.div key={step.title} initial={{opacity: 0, y: 20}} whileInView={{opacity: 1, y: 0}} viewport={{ once: true, amount: 0.5 }} transition={{duration: 0.5, delay: index * 0.2}} className="flex flex-col items-center p-6 bg-white rounded-xl shadow-md">
...
const ReportModal = ({ isOpen, onClose, tips, user, onUnlockRequest }: ReportModalProps) => {
    ...
    return (
        <AnimatePresence>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                    <div className="p-6 border-b flex justify-between items-center">
                        <h2 className="text-xl font-bold">Your Personalized Report</h2>
                        <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full"><X size={24} /></Button>
                    </div>
                    <div className="p-6 overflow-y-auto relative">
                        {tips.length === 0 ? ( <p className="text-green-600 font-semibold">Great job—all fields meet or exceed targets!</p> ) : (
                            <ul className="space-y-3 list-disc pl-5">
                                {tips.map((tip, i) => ( <li key={i} dangerouslySetInnerHTML={{ __html: tip }} className={!isUnlocked && i >= initialTipsCount ? 'blur-sm select-none pointer-events-none' : ''}/> ))}
                            </ul>
                        )}
                        {!isUnlocked && tips.length > initialTipsCount && (
                            <div className="absolute bottom-0 left-0 w-full h-48 bg-gradient-to-t from-white via-white/90 to-transparent flex flex-col justify-end items-center text-center p-6 pointer-events-none">
                                <div className="pointer-events-auto">
                                  <motion.button onClick={onUnlockRequest} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="flex flex-col items-center text-primary font-semibold hover:text-primary/80">
                                      <span>Unlock {tips.length - initialTipsCount} More Tips</span>
...
export default function FinancialHealthCheckupPage() {
    ...
    const computeRating = useCallback((f: NumericField): number => {
        ...
        if (currentField.key === "debtManagement") {
            ...
            if (maxEMI <= 0 && actualEMI > 0) return 1;
    ...
    const { finalScore, numericScoreWeighted, yesScoreWeighted, allTips, numericComplete } = useMemo(() => {
        ...
        const yNoTips: string[] = yesNoState.filter(y => y.value === 'no' || y.value === 'dont_know').map(y => `Consider addressing <strong>"${y.label}"</strong>. ${y.info}`);
        return { finalScore: finScore, numericScoreWeighted: numScoreWeighted, yesScoreWeighted: yScoreWeighted, allTips: [...numTips, ...yNoTips], numericComplete: numComplete };
