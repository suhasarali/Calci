...
type LabeledInputProps = { label: string; value: string; onChange: (v: string) => void; info: string; placeholder?: string; isDesktop: boolean; };
...
export default function OnePageFinancialRoadmap() {
  ...
  const calculations = useMemo(() => {
    ...
    const postRetRate = Number(retPost)/100;
    const realRatePostRetirement = ((1 + postRetRate) / (1 + inflationRate)) - 1;

    const retirementCorpusRequired = realRatePostRetirement <= 0
        ? number(yearlyExpenseAtRetirement * postRetYears)
        : number(Math.abs(PV(realRatePostRetirement, postRetYears, yearlyExpenseAtRetirement, 0, 1)));
    ...
    const annualExpenseToday = monthlyExpensesNum * 12;
    const nperSpouse = Math.max(0, Number(lifeExpectancySpouse) - Number(spouseAge));
    const claimReturnRate = Number(claimReturn) / 100;
    const realRateClaim = ((1 + claimReturnRate) / (1 + inflationRate)) - 1;
    ...
    const pvOfGoals = goalsWithCalc.reduce((acc, goal) => {
        const pvGoal = Math.abs(PV(claimReturnRate, Number(goal.years), 0, -goal.futureValue, 0));
        return acc + pvGoal;
    }, 0);
