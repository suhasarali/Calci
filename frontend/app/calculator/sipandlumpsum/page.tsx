...
// --- MAIN PAGE COMPONENT ---
export default function InvestmentInflationCalculator() {
    return (
        <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
            <NavbarHome />
            <div className="max-w-6xl mt-18 mx-auto">
                <Card className="bg-white shadow-xl rounded-2xl border-gray-200">
                    <CardHeader className="border-b-2 border-gray-100 p-6">
                        <BackButton />
                        <CardTitle className="text-3xl font-bold text-gray-800 text-center">Investment Inflation Calculator</CardTitle>
                    </CardHeader>
                    <CardContent className="p-6 md:p-8">
...
function SIPCalculator() {
    ...
    const [interestRate, setInterestRate] = useState(12);
    const [investmentTenure, setInvestmentTenure] = useState(5);
    const [inflationRate, setInflationRate] = useState(6);
    const [showFutureValue, setShowFutureValue] = useState(false);
    const [futureValueYear, setFutureValueYear] = useState(1);
    ...
    const { primaryCalculations, yearlyData } = useMemo(() => {
        ...
        const tenureInMonths = investmentTenure * 12;
        const annualInflation = inflationRate / 100;

        const generatedWealth = p * ((Math.pow(1 + monthlyRate, tenureInMonths) - 1) / monthlyRate) * (1 + monthlyRate);
        const investedAmount = p * tenureInMonths;
        const totalInterest = generatedWealth - investedAmount;
        const inflationAdjustedWealth = generatedWealth / Math.pow(1 + annualInflation, investmentTenure);

        const yearlyData = Array.from({ length: investmentTenure + 1 }, (_, i) => {
            const year = i;
            const months = year * 12;
            const fv = months > 0 ? p * ((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate) * (1 + monthlyRate) : 0;
            const realFv = fv / Math.pow(1 + annualInflation, year);
            const totalInvested = p * months;
            return { year: `Year ${year}`, 'Generated Wealth': fv, 'Inflation-Adjusted Wealth': realFv, 'Invested Amount': totalInvested };
        });

        return {
            primaryCalculations: { investedAmount, generatedWealth, inflationAdjustedWealth, totalInterest },
            yearlyData,
        };
    }, [monthlyInvestment, interestRate, investmentTenure, inflationRate]);

    const futureValueCalculations = useMemo(() => {
        if (!showFutureValue || futureValueYear <= 0 || futureValueYear > investmentTenure) return { futureValue: 0, inflationAdjustedFutureValue: 0, investedAmount: 0 };
        
        const p = monthlyInvestment;
        const monthlyRate = (interestRate / 100) / 12;
        const annualInflation = inflationRate / 100;
        const months = futureValueYear * 12;

        const futureValue = p * ((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate) * (1 + monthlyRate);
        const inflationAdjustedFutureValue = futureValue / Math.pow(1 + annualInflation, futureValueYear);
        const investedAmount = p * months;

        return { futureValue, inflationAdjustedFutureValue, investedAmount };
    }, [monthlyInvestment, interestRate, inflationRate, showFutureValue, futureValueYear, investmentTenure]);

    return (
        <div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 mb-8">
                <InputWithIcon Icon={RupeeIcon} label="Monthly Investment (₹)" value={formatCurrency(monthlyInvestment)} onChange={(e) => { const num = parseCurrency(e.target.value); if (!isNaN(num)) setMonthlyInvestment(num); }} subText={`${capitalize(toWords(monthlyInvestment))} Rupees`}/>
                <InputWithIcon Icon={CalendarIcon} label="Investment Tenure (Years)" type="number" min={1} max={50} value={investmentTenure} onChange={(e) => setInvestmentTenure(Number(e.target.value))}/>
                <InputWithIcon Icon={PercentIcon} label="Expected Return Rate (% p.a.)" type="number" min={1} max={30} step={0.5} value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))}/>
                <InputWithIcon Icon={TrendingUpIcon} label="Inflation Rate (% p.a.)" type="number" min={1} max={20} step={0.5} value={inflationRate} onChange={(e) => setInflationRate(Number(e.target.value))}/>
            </div>
...
const CalculatorResults = ({ primaryCalculations, yearlyData, showSpecificYear, specificYear, specificYearCalculations, isSip }) => {
    // Prepare data for the specific year result card based on whether it's SIP or Lumpsum
