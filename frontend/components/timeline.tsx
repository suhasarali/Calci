...
export function Timeline() {
  ...
  return (
    ...
    >
      <h2 className="text-4xl sm:text-6xl text-center font-bold mb-5 text-gray-800">
        How It <span className="text-[#0065FF]">Works?</span>
      </h2>
      <h3 className="text-xl sm:text-2xl text-center font-semibold mb-6 text-gray-700">Four simple steps to financial clarity</h3>
      ...
      <div className="sm:hidden">
        <div className="relative max-w-md mx-auto">
          {/* Vertical Line */}
          <div className="absolute left-5 top-0 bottom-0 w-1 bg-[#CCE0FF]">
            <div
              className="absolute top-0 left-0 w-full bg-[#0065FF] transition-all duration-300"
              style={{
                height: `${scrollPercentage}%`,
              }}
            />
            ...
            {timelineData.map((item, index) => {
              ...
              return (
                <div key={index} className="relative flex items-start pl-16">
                  ...
                  <div
                    className={`absolute left-0 z-10 flex items-center justify-center w-12 h-12 bg-white border-4 rounded-full transition-colors duration-300 ${
                      isActive ? 'border-[#0065FF]' : 'border-[#CCE0FF]'
                    }`}
                  >
                    <IconComponent
                      className={`transition-colors duration-300 ${
                        isActive ? 'text-[#0065FF]' : 'text-[#CCE0FF]'
                      }`}
                      size={24} // 3. Use size prop for lucide-react icons
                    />
                  </div>
                  ...
                  <div
                    className={`bg-white p-6 rounded-lg shadow-lg transition-all duration-300 relative ${
                      isActive ? 'shadow-[#CCE0FF]' : ''
                    }`}
                  >
                    <span className="absolute text-6xl font-bold text-[#E6F0FF]/80 z-0 right-4 -top-2">
                      {item.step}
                    </span>
                    <div className="relative z-10">
                      <h3 className="text-xl font-bold text-gray-800 mb-2">
      ...
      <div className="hidden sm:block">
        <div className="relative max-w-6xl mx-auto px-4">
          {/* Horizontal Line Container */}
          <div className="relative mb-24">
            <div className="absolute left-0 right-0 top-6 h-1 bg-[#CCE0FF]">
              <div
                className="absolute left-0 top-0 h-full bg-[#0065FF] transition-all duration-300"
                style={{
                  width: `${scrollPercentage}%`,
                }}
              />
